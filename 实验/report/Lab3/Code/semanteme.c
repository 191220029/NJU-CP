...
void initTable(){
   ...
   for(int i = 0; i <= HASHSIZE ; i++)
    ...
    HashTable* readFunc = (HashTable*)malloc(sizeof(HashTable));
    readFunc->size = 0;
    readFunc->next = readFunc->down = readFunc->stackRoot = NULL;
    readFunc->isStackNode  = false;
    readFunc->name = "read";
    readFunc->type = (sType*)malloc(sizeof(sType));
    readFunc->type->kind = sFUNCTION;
    readFunc->type->u.function.argc = 0;
    readFunc->type->u.function.argList = NULL;
    readFunc->type->u.function.type = (sType*)malloc(sizeof(sType));
    readFunc->type->u.function.type->kind = sINT;
    readFunc->operand = NULL;
    hash_add(readFunc);
    
    HashTable* writeFunc  = (HashTable*)malloc(sizeof(HashTable));
    writeFunc->size = 0;
    writeFunc->next = writeFunc->down = writeFunc->stackRoot = NULL;
    writeFunc->isStackNode = false;
    writeFunc->name = "write";
    writeFunc->type = (sType*)malloc(sizeof(sType));
    writeFunc->type->kind = sFUNCTION;
    FieldList* arg = (FieldList*)malloc(sizeof(FieldList));
   arg->name = "";
   arg->funcVarFlag = arg->funDecFlag = false;
   arg->tail = NULL;
   arg->type = (sType*)malloc(sizeof(sType));
   arg->type->kind = sINT;

    writeFunc->type->u.function.argc = 1;
    writeFunc->type->u.function.argList = arg;
    writeFunc->type->u.function.type = (sType*)malloc(sizeof(sType));
    writeFunc->type->u.function.type->kind = sINT;
    writeFunc->operand = NULL;
    hash_add(writeFunc);
    return;
}
